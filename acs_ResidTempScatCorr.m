function acs_ResidTempScatCorr(dir_acs,acs,TScoeff)

% This function corrects the residual temperature and scattering errors of
% ap and residual temperature of cp from AC-S inline system. Residual
% temperature correction of ap and cp follows Slade et al (2010), and
% scattering errors correction of ap follows Zaneveld et al (1994).
...
for k=1:size(ap,1)
    ...
    if all(isfinite(ap(k,:)))
        ...
        ap_TSalScatCorr(k,:)=ap(k,:)-psi_T.*deltaT(k)- ...
            ((ap(k,pos_ref)-psi_T(pos_ref).*deltaT(k))./bp(pos_ref)).*bp;
        
        cp_ScatCorr(k,:)=cp(k,:)-psi_T.*deltaT(k);
...
bp=cp_ScatCorr-ap_TSalScatCorr;
...
ap_TSalScatCorr(pos,:)=[]; cp_ScatCorr(pos,:)=[]; 
t(pos,:)=[]; t2(pos,:)=[];
deltaT(pos,:)=[]; fiterr(pos,:)=[];

acs_TSalScatCorr.ap=ap_TSalScatCorr;
acs_TSalScatCorr.cp=cp_ScatCorr;
acs_TSalScatCorr.t=t;
acs_TSalScatCorr.t2=t2;
acs_TSalScatCorr.wl=wl;
acs_TSalScatCorr.deltaT=deltaT;
acs_TSalScatCorr.fit_error=fiterr;

namestr2=[strcat(dir_acs,'acs_TSalScatCorr'),'.mat'];
save (namestr2,'acs_TSalScatCorr')
...
costf=sum( abs(ap(pos_NIR) - psi_T(pos_NIR).*delT - ((ap(pos_ref)-psi_T(pos_ref).*delT)./bp(pos_ref)).*bp(pos_NIR)) );
